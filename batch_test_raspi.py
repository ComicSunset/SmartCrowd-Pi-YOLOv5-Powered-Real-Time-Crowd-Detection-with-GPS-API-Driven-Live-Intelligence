...
# === Configuration ===
ip_stream_url = "http://192.168.0.141:5000/video"  # Replace with your Pi IP stream
...
# === Video Capture from IP Stream ===
cap = cv2.VideoCapture(ip_stream_url)

if not cap.isOpened():
    print("Error: Could not open video stream. Check your IP address or stream source.")
    exit()

# Output video writer (optional – comment if not needed)
width = int(cap.get(cv2.CAP_PROP_FRAME_WIDTH)) or 640
height = int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT)) or 480
fps = int(cap.get(cv2.CAP_PROP_FPS)) or 15
output_video_path = r'D:\YOLO-CROWD\output_video_from_stream.avi'
